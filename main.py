...
def checkGuess(turns, word, userGuess, window):
    ...
    for x in range(0,5):
        ...
        rect = pygame.Rect(60 +spacing, 50+ (turns*80), 50, 50)
        pygame.draw.rect(window, guessColourCode[x], rect)
        window.blit(renderList[x], (75 + spacing, 60 + (turns*80)))
        spacing+=80
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key != pygame.K_BACKSPACE:
                ...
                if event.key == K_RETURN and win == True: #game already won and they try to play more
                    main()

                if event.key == K_RETURN and turns == 6: #game already lost and they try to play more
                    main()

                if event.key == K_RETURN and len(guess) > 4: #put in a proper word and pressed enter
                    win = checkGuess(turns, word, guess, window)
                    turns+=1
                    guess = ""
                    window.fill(black,(0,500, 500, 200))

        window.fill(black,(0,500, 500, 200)) #always updating the screen when a new word is added)
